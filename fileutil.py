...
def read_set_of_lists_from_tsv(filename, separator='\t', list_type=tuple):
    """The default here is tuple instead of list, because you can't add lists
    to a set, only tuples."""
    s = set()
    for line in open(filename):
...
def write_set_to_file(filename, set_to_write):
    out = create_dirs_and_open(filename)
